...
When the app crashes or closes on launch, read what the device recorded before guessing from screenshots. An iOS session keeps each crash report with its stack and the app's last log lines, plus a buffer of the device log. Read them with `node <skill-dir>/scripts/preview-api.js get <route>` from the project directory. The script keeps the session token out of your output.

**Start `node <skill-dir>/scripts/preview-api.js watch --seconds 120` before you reproduce the crash.** Without it, the crash gets a report but an empty log tail. Commands, fields, and fallbacks are in [references/logs-and-crashes.md](./references/logs-and-crashes.md).
