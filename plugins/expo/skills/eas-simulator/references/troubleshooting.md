...
| argent `screenshot` crashes on the VM (`NSPasteboard`) | Worker runs under a System LaunchDaemon session (no WindowServer) | Use `agent-device`, or file an issue upstream. |
