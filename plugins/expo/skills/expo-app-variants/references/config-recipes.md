...
// Optional, and only when the project sets a `scheme` — see Step 3, question 3.
// Splitting it stops the OS picking a winner between variants, and costs one
// registration per variant in every service that knows the scheme.
...
- `config.scheme` is typed `string | string[]`, hence the `typeof` guard. Drop the `scheme` line unless the user opted into a per-variant scheme, and always drop it when the config sets no scheme.
...
A per-variant `scheme` takes the same shape here, when the user opted into one. Take `getScheme()` from Recipe 1 and call it with the literal base, since nothing is passed in here: `scheme: getScheme("my-app")`.
