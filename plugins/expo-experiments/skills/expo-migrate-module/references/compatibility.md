...
| Shared-object static members | `@JS static` | `_decorateSharedObject(constructor:)`, absent from released core as of 2026-09-09 | calling the member on the JS class itself, not an instance |
...
| Free-form `Any` arguments | `@JS` with `Any`, `[Any]`, `[String: Any]` | `decodeAny`, `decodeAnyArray`, `decodeAnyDictionary` | building it, then passing a JS object through (plugin `v0.9.0`) |
...
- Shared-object `@JS static`/`class` members bind onto the constructor object through a `constructor:`-labeled decoration hook, separate from the `prototype:` one used for instance members. The plugin has emitted this binding since `v0.7.0`, but the released core declared only the `prototype:` overload as of 2026-09-09, so a static member expands and then fails to link. Instance support does not imply static support. Keep `StaticFunction`/`StaticAsyncFunction` entries in the 1.0 `Class(...)` block until the hook is present.
...
## Landed in the macros plugin since v0.8.0

These arrived in the macros plugin after the skill's original survey. All of them are macro-side only: the plugin generates the code, but the paired support in `expo/expo` had not landed as of 2026-09-09. Verify the checked-out core before relying on any of them.

Released in `0.9.0`: free-form `Any` arguments and `scan-modules`, both covered below. Merged for `0.10.0` but untagged as of 2026-09-09: `@JS(.concurrent)` (see the threading section in `references/migration-map.md`) and `@Union` (see the unions section). Because `0.10.0` was unreleased, an installed dependency will not have either; check the checked-out macro declarations rather than assuming a version.

### Free-form `Any` arguments in `@JS` (plugin `v0.9.0`)
...
The decoding entry points these bindings rely on (`JavaScriptValue.decodeAny`, `decodeAnyArray`, `decodeAnyDictionary`) had not landed in core as of 2026-09-09, so such a member expands and then fails to link. Confirm by building one minimal free-form argument against the checked-out core before migrating any others.
...
### `scan-modules` for autolinked module discovery (plugin `v0.9.0`)
...
The intent is that `expo-modules-autolinking` detects `@ExpoModule` classes automatically, replacing the `expo-module.config.json` module list. **That consumer side had not landed in `expo/expo` as of 2026-09-09.** Consequences for a migration:
